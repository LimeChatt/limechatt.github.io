(async function() {
    ...
    server.onmessage = async (msg) => {
        ...
        const parsedData = JSON.parse(await msg.data)

        if(parsedData.type == 'message') {
            createMessage(parsedData.author, parsedData.content, parsedData.date ? new Date(parsedData.date) : new Date('11/13/1987'), parsedData.attachment ? parsedData.attachment : '')
        }

        if(parsedData.response) {
            parsedData.response.forEach(async (msg) => {
                if(msg.type == 'message') {
